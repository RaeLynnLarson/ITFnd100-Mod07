...
-- 2017-01-01,Rachel Larson,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_RachelLarson')
	 Begin 
	  Alter Database [Assignment07DB_RachelLarson] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_RachelLarson;
	 End
	Create Database Assignment07DB_RachelLarson;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_RachelLarson;
...
select top 1000000000
	ProductName,
	format(UnitPrice, 'C', 'en-US') as US_Price
from vProducts
...
Select top 1000000
	ProductName, 
	 DateName(mm, InventoryDate) + ', ' + cast(datePart ("Year", InventoryDate) as varchar(50)) as InventoryDate ,
...
select top 10000000
	...
	IsNull(lag(sum(I.count)) over (order by ProductName, Month(InventoryDate)), 0) as PreviousMonthCounts
	from vProducts as P 
	join vInventories as I on P.ProductID = I.ProductID
	group by ProductName, I.InventoryDate
	order by ProductName, I.InventoryDate;
	go
...
Select top 1000000
	...
	[CountVsPreviousCountKPI] = case
		When [InventoryCount] > [PreviousMonthCounts] then 1
		when [InventoryCount] = [PreviousMonthCounts] then 0
		when [InventoryCount] < [PreviousMonthCounts] then -1
		end
